...
const form = ref({ ...props.supplier })
const errors = ref({})
const processing = ref(false)

function update() {
    processing.value = true
    router.put(`/suppliers/update/${form.value.id}`, form.value, {
        onSuccess: () => {
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form @submit.prevent="update" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Supplier Name <span
                                class="text-red-500">*</span></label>
                        <input id="name" v-model="form.name" type="text" required
                            class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            :class="{ 'border-red-500': errors.name }" placeholder="Enter supplier name" />
                    ...
                    <div>
                        ...
                        <input id="company_name" v-model="form.company_name" type="text"
                            class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            :class="{ 'border-red-500': errors.company_name }" placeholder="Company name" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input id="phone" v-model="form.phone" type="tel" required
                            class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            :class="{ 'border-red-500': errors.phone }" placeholder="e.g. 03001234567" />
                    ...
                    <div>
                        ...
                        <input id="email" v-model="form.email" type="email"
                            class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            :class="{ 'border-red-500': errors.email }" placeholder="supplier@example.com" />
                ...
                <div>
                    ...
                    <textarea id="address" v-model="form.address" rows="3"
                        class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        :class="{ 'border-red-500': errors.address }" placeholder="Full supplier address"></textarea>
                    <p v-if="errors.address" class="mt-1 text-sm text-red-600">{{ errors.address }}</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="balance_due" class="block text-sm font-medium text-gray-700 mb-1">Due
                            Balance</label>
                        <input id="balance_due" v-model="form.balance_due" type="number" step="0.01" min="0"
                            class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            :class="{ 'border-red-500': errors.balance_due }" placeholder="0.00" />
